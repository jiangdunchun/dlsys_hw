...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        node.grad = sum(grads)
        if node.op != None:
            out_grads = node.op.gradient(node.grad, node)
            for input, out_grad in zip(node.inputs, out_grads):
                if input not in node_to_output_grads_list: node_to_output_grads_list[input] = list()
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    visited.add(node)
    for input in node.inputs: topo_sort_dfs(input, visited, topo_order)
